...
public class MusicPlayer implements Runnable
{
	...
	/**
	 * @param deviceIp 目标ip
	 * @param srcChannelType 声源类型
 	 * @param dstPort 目标端口
	 */
	public MusicPlayer(String deviceIp, int srcChannelType, int dstPort)
	{
	...
	class BufferThread implements Runnable
	{
		...
		int channelID;
		public BufferThread(ChannelType channelType,int channelID)
		{
		...
		public void run()
		{
			while (AudioCommand.isSend)
			{
				bufferSize = controlCommand.getCurrentBufferNum(channelType,channelID);
	 ...
	 * 改变声道
	 * @param deviceIp
	 * @param srcChannelType
	 * @param dstPort
	 */
	public void changeChannel(String deviceIp, int srcChannelType, int dstPort)
	{
		this.deviceIp = deviceIp;
		this.srcChannelType = srcChannelType;
		this.dstPort = dstPort;
	...
	public byte[] byte2MusicCommand(byte[] audioData)
	{
	   /*byte[] datas = new byte[1242];
		...
		System.arraycopy(bitDepth, 0, datas, 35, 1);
		System.arraycopy(timeStamp, 0, datas, 36, 4);
		System.arraycopy(audioData, 0, datas, 50, 1200);// 下标40开始
		System.arraycopy(end, 0, datas, 1250, 2);
		return datas;
	...
	/** 命令 */
	byte[] cmd = new byte[]{0x30};//0x30 音频协议
	/** 本机IP */
	...
	{
		//判断音乐格式
		String fileFormat = url.substring(url.lastIndexOf(".") + 1).toUpperCase();
		...
		if ("MP3".equals(fileFormat) || "WMA".equals(fileFormat))
		{
			...
			new Thread()
			{
				public void run()
				{
					while (AudioCommand.isSend)
					{
						bufferSize = controlCommand.getCurrentBufferNum(deviceIp, dstPort);
						try
						{
							Thread.sleep(500);
						} catch (InterruptedException e)
			...
			//声道类型
			int channelTypeInt=srcChannelType==0?lEFT:RIGHT;
			 //==================================================================开始编码==================================================================
					...
					if (res >= 0)
					{
						...
						{								
							...
							if (decodeBuffer.length ==0)
							{
								...
								{
									Log.w(TAG, "bufferSize=" + bufferSize);
									...
									try
									{	
										sendAudio(datas,  dstPort,deviceIp);																											
									} catch (Exception e1)
									{
